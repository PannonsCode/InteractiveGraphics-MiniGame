...
class World2{
	...
	constructor(camera){
		...
		this.stone1 = new Stone(30,2,-50,'textures/orange.jpg');
		this.stone1.stone.name = "stone1";
		this.stone2 = new Stone(-20,2,-20,'textures/yellow.jpg');
		this.stone2.stone.name = "stone2";
		this.tot_stones = 2;
		this.accumulated_stones = 0;
		this.scene = this.set_world2();
		this.camera={
			x: 1,
			y: 3,
			z: 5
	...
	set_world2(){
		...
		const hemiLight = new THREE.HemisphereLight( 'darkorange', 'gray' );
		...
		new Model3D(scene,'models/city.glb',0,0.2,0);
		new Model3D(scene,'models/city.glb',400,0.2,0);
		new Model3D(scene,'models/city.glb',0,0.2,400);
		new Model3D(scene,'models/city.glb',400,0.2,400);
		
		window.alert("GOING IN THE SECOND WORLD, LOOK FOR THE 2 STONES!!! \n (Hint: look on the left and on the right)");
		return scene;
	}
	...
	animate(){
		...
		if(this.portal.open && this.portal.portal.position.x==this.scene_camera.position.x && this.portal.portal.position.z==this.scene_camera.position.z){
			Util.world = new World3(this.scene_camera);
